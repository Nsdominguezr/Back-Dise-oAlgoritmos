"""Módulo de pruebas de autenticación.

Contiene pruebas unitarias y de integración para el servicio de identidad.
"""
...
def test_hashing_bcrypt():
    ...
    assert bcrypt.checkpw(password_plana, hashed_pw) is True

    # Validar que devuelva False para una clave incorrecta.
    assert bcrypt.checkpw("ClaveMala".encode('utf-8'), hashed_pw) is False
...
def client():
    ...
    Args:
        app_test: Instancia de la app exclusiva para pruebas.

    Returns:
        Cliente de pruebas con contexto de aplicación.
    """
    # 1. Creamos una instancia de la app exclusiva para pruebas.
    app_test = create_app()
    app_test.config['TESTING'] = True

    # 2. PRO-TIP: Usamos SQLite en memoria para no tocar tu MySQL
    # durante las pruebas.
...
def test_login_integracion_fallido(client):
    ...
    # Verificar que el Criterio de Aceptación se cumple:
    # "Fallido devuelve error 401".
