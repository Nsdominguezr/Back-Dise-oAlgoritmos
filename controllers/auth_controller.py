...
Gestiona el registro, login, refresh tokens y gestión de usuarios.
"""
...
def admin_global_required(f):
    """Decorador para requerir rol Admin Global en el endpoint.

    Args:
        f: Función a decorar.

    Returns:
        Función decorada que verifica el token JWT y el rol.
    """
    @wraps(f)
    def decorated(*args, **kwargs):
        ...
        try:
            ...
            if data.get('rol') != 'Admin Global':
                return jsonify(
                    {'mensaje': 'Acceso denegado. Requiere privilegios de Admin Global.'}
                ), 403
...
def get_usuarios():
    ...
    """
    # Retornamos TODOS los usuarios (activos e inactivos) para que
    # el Admin pueda ver el historial completo.
...
def registrar_usuario():
    ...
    Args:
        None (datos en JSON: username, password, rol_id, sede_id).

    Returns:
        Usuario creado en formato JSON o mensaje de error.
    """
    data = request.get_json()
    if not data or not data.get('username') or not data.get('password') \
            or not data.get('rol_id') or not data.get('sede_id'):
    ...
    try:
        ...
        db.session.rollback()
        return jsonify(
            {'mensaje': 'Error al crear usuario, es posible que el username ya exista.'}
        ), 500
...
def login():
    ...
    Args:
        None (datos en JSON: username, password).

    Returns:
        Dict con mensaje, token, refresh_token, expira_en y datos de usuario.
    """
    data = request.get_json()
    if not data or not data.get('username') or not data.get('password'):
...
def refresh():
    ...
    Args:
        None (datos en JSON: refresh_token).

    Returns:
        Dict con mensaje, nuevo token y fecha de expiración.
    """
    data = request.get_json()
    ...
    try:
        ...
        if not usuario.activo:
            return jsonify(
                {'mensaje': 'La cuenta ha sido suspendida durante su sesión.'}
            ), 403
        ...
        return jsonify(
            {'mensaje': 'El Refresh Token ha expirado. Inicie sesión nuevamente.'}
        ), 401
    except jwt.InvalidTokenError:
...
def desactivar_usuario(usuario_id):
    """Desactiva (soft delete) un usuario del sistema.

    El usuario no se borra, pero no podrá hacer login.
    ...
    # Si quieres implementar esto más riguroso,
    # deberías extraer el ID del JWT del token entrante.
    if usuario.rol.nombre == 'Admin Global':
        return jsonify(
            {'mensaje': 'Advertencia: No puedes desactivar a un Admin Global desde aquí.'}
        ), 400
    ...
    return jsonify(
        {'mensaje': f'El usuario {usuario.username} ha sido dado de baja. '
                    'Se ha revocado su acceso.'}
    ), 200
...
def get_reporte(tipo):
    ...
    Args:
        tipo: Tipo de reporte ('inventario' o 'financiero').

    Returns:
        Archivo CSV con el reporte solicitado.
    """
    ...
    if tipo == 'inventario':
        ...
        return jsonify(
            {'mensaje': 'Tipo de reporte no válido. Opciones: inventario, financiero'}
        ), 400